...
class FormSkckController extends Controller
{
    ...
    public function edit($id) {
        ...
        return view('mazer_template.admin.form_skck.edit', compact('imagePath', 'skck_daftar_diris', 'skck_daftar_saudara_kandungs', 'skck_riwayat_sekolahs', 'skck_saudara_yg_menjadi_tanggungans', 'skck_daftar_anaks'));
    ...
    public function update(Request $request, $id) {
        try {
            ...
            Alert::error('Gagal update form permohonan skck!');
            return redirect()->route('admin.formskck.index');
        } catch (ModelNotFoundException $e) {
            Alert::error('Gagal update form permohonan skck!');
            return redirect()->route('admin.formskck.index');
        ...
        // Check if the 'no_kitas' values have changed
        if ($request->input('skck_daftar_diris_no_kitas') !== $Skck->no_kitas) {
            $validator->addRules(['skck_daftar_diris_no_kitas' => 'required|unique:skck_daftar_diris,no_kitas']);
        }

        // Check if the 'no_passport' values have changed
        if ($request->input('skck_daftar_diris_no_passport') !== $Skck->no_passport) {
            $validator->addRules(['skck_daftar_diris_no_passport' => 'required|unique:skck_daftar_diris,no_passport']);
        }

        // Check if the 'nik' values have changed
        if ($request->input('skck_daftar_diris_nik') !== $Skck->nik) {
            $validator->addRules(['skck_daftar_diris_nik' => 'required|unique:skck_daftar_diris,nik']);
        }

        if($validator->fails()) {
            Alert::error('Cek kembali pengisian form, terima kasih !');
            // Alert::error($validator->errors());
            return redirect()->route('admin.formskck.edit',$id)->withErrors($validator->errors())->withInput();
        }
        ...
        try {
            ...
            // setp 1: create skck_daftar_diris
            DB::table('skck_daftar_diris')->Where('id',$id)
                ->update([
                'nama' => $request->input('skck_daftar_diris_nama'),
                'keperluan' => $request->input('skck_daftar_diris_keperluan'),
                'tempat_lahir' => $request->input('skck_daftar_diris_tempat_lahir'),
            ...
            if (count($skck_daftar_saudara_kandungs_namas) > 0) {
                for ($i = 0; $i < count($skck_daftar_saudara_kandungs_namas); $i++) {
                    $saudara_kandung_id = DB::table('skck_daftar_saudara_kandungs')
                        ->where('skck_daftar_diris_id', $id)
                        ->select('id')
                        ->get()
                        ->pluck('id')
                        ->toArray();

                    if (isset($saudara_kandung_id[$i])) {
                        DB::table('skck_daftar_saudara_kandungs')
                            ->where('id', $saudara_kandung_id[$i])
                            ->update([
                                'nama' => $skck_daftar_saudara_kandungs_namas[$i],
                                'umur' => $skck_daftar_saudara_kandungs_umurs[$i],
                                'pekerjaan' => $skck_daftar_saudara_kandungs_pekerjaans[$i],
            ...
            if (count($skck_riwayat_sekolahs_nama_pendidikans) > 0) {
                for ($i = 0; $i < count($skck_riwayat_sekolahs_nama_pendidikans); $i++) {
                    $riwayat_sekolah_id = DB::table('skck_riwayat_sekolahs')
                        ->where('skck_daftar_diris_id', $id)
                        ->select('id')
                        ->get()
                        ->pluck('id')
                    ...
                    if (isset($riwayat_sekolah_id[$i])) {
                        DB::table('skck_riwayat_sekolahs')->where('id', $riwayat_sekolah_id[$i])
                        ->update([
                            'nama_pendidikan' => $skck_riwayat_sekolahs_nama_pendidikans[$i],
                            'tahun_lulus' => $skck_riwayat_sekolahs_tahun_luluss[$i],
                        ]);
            ...
            if (count($skck_saudara_yg_menjadi_tanggungans_namas) > 0) {
                for ($i = 0; $i < count($skck_saudara_yg_menjadi_tanggungans_namas); $i++) {
                    $saudara_yg_menjadi_tanggungan_id = DB::table('skck_saudara_yg_menjadi_tanggungans')
                        ->where('skck_daftar_diris_id', $id)
                        ->select('id')
                        ->get()
                        ->pluck('id')
                    ...
                    if (isset($saudara_yg_menjadi_tanggungan_id[$i])) {
                        DB::table('skck_saudara_yg_menjadi_tanggungans')->where('id', $saudara_yg_menjadi_tanggungan_id[$i])
                        ->update([
                            'nama' => $skck_saudara_yg_menjadi_tanggungans_namas[$i],
                            'alamat' => $skck_saudara_yg_menjadi_tanggungans_alamats[$i],
                        ]);
            ...
            if (count($skck_daftar_anaks_namas) > 0) {
                for ($i = 0; $i < count($skck_daftar_anaks_namas); $i++) {
                    $skck_daftar_anaks_id = DB::table('skck_daftar_anaks')
                        ->where('skck_daftar_diris_id', $id)
                        ->select('id')
                        ->get()
                        ->pluck('id')
                    ...
                    if (isset($skck_daftar_anaks_id[$i])) {
                        DB::table('skck_daftar_anaks')->where('id',$skck_daftar_anaks_id[$i])
                        ->update([
                            'nama' => $skck_daftar_anaks_namas[$i],
                            'umur' => $skck_daftar_anaks_umurs[$i],
                        ]);
            ...
            // Alert::error($e->getMessage());
            Alert::error('Gagal simpan form skck!');
            return redirect()->route('sipastap.formskck.edit',$id);
        ...
        Alert::success('Sukses', 'Update form permohonan skck berhasil.');
        return redirect()->route('admin.formskck.index');
    }
